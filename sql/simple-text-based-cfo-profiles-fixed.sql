-- シンプルなテキストベースCFOプロフィール実装（UUID修正版）
-- 原文保持 + LIKE検索で柔軟性を実現
...
海外を絡ませた案件も得意。特にUSでのIPOサポートはNYの投資銀行と一緒に可能。',
        ...
        career_full_text = EXCLUDED.career_full_text,
        skills_full_text = EXCLUDED.skills_full_text,
        qualifications_text = EXCLUDED.qualifications_text,
        introduction_full_text = EXCLUDED.introduction_full_text,
...
CREATE INDEX IF NOT EXISTS idx_rextrix_cfos_career_text_gin 
ON rextrix_cfos USING GIN (to_tsvector('japanese', career_full_text));

CREATE INDEX IF NOT EXISTS idx_rextrix_cfos_skills_text_gin 
ON rextrix_cfos USING GIN (to_tsvector('japanese', skills_full_text));

-- Phase 5: 柔軟な検索クエリのテスト
-- 検索例1: M&A経験者を探す
SELECT 
    up.display_name,
    c.title,
    c.experience_years,
    CASE 
        WHEN c.career_full_text ILIKE '%M&A%' THEN '✅ M&A経験'
        ELSE ''
    END as ma_experience,
    CASE 
        WHEN c.skills_full_text ILIKE '%IPO%' THEN '✅ IPO対応'
        ELSE ''
    END as ipo_capability
FROM rextrix_cfos c
...
SELECT 
    ...
    c.title,
    CASE 
        WHEN c.career_full_text ILIKE '%海外%' THEN '✅ 海外経験'
        WHEN c.career_full_text ILIKE '%フィリピン%' THEN '✅ フィリピン経験'
        WHEN c.career_full_text ILIKE '%アメリカ%' OR c.career_full_text ILIKE '%US%' THEN '✅ 米国経験'
        ELSE ''
    END as international_experience
FROM rextrix_cfos c
LEFT JOIN rextrix_user_profiles up ON c.user_id = up.user_id
WHERE c.career_full_text ILIKE '%海外%' 
   OR c.career_full_text ILIKE '%フィリピン%'
   OR c.career_full_text ILIKE '%US%'
   OR c.career_full_text ILIKE '%アメリカ%'
...
FROM rextrix_cfos c
LEFT JOIN rextrix_user_profiles up ON c.user_id = up.user_id
WHERE c.qualifications_text ILIKE '%診断士%' 
   OR c.qualifications_text ILIKE '%簿記%'
   OR c.qualifications_text ILIKE '%会計士%'
ORDER BY c.experience_years DESC;
