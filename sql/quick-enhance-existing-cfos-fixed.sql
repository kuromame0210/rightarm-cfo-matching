...
UPDATE rextrix_cfos SET 
    work_experiences = jsonb_build_array(
        jsonb_build_object(
    ...
    detailed_certifications = jsonb_build_array(
        jsonb_build_object(
            'name', '米国公認会計士',
            'organization', 'AICPA',
            'is_active', true,
    ...
    compensation_details = jsonb_build_object(
        'primary_type', 'monthly',
        'monthly_rate', jsonb_build_object('min', 150000),
    ...
    specialties = jsonb_build_array('事業計画作成', '財務モデル作成', '資金調達戦略', 'M&A支援', '経営管理')
...
UPDATE rextrix_cfos SET 
    work_experiences = jsonb_build_array(
        ...
        jsonb_build_object(
            ...
            'industry', '金融・VC',
            'experience_years', 6,
            'achievements', jsonb_build_array('50社以上のスタートアップへ投資', 'ハンズオン支援実績'),
            'is_current', false,
            'display_order', 2
        ),
        jsonb_build_object(
    ...
    compensation_details = jsonb_build_object(
        'primary_type', 'monthly',
        'monthly_rate', jsonb_build_object('min', 50000),
        'project_based', true,
        'currency', 'JPY'
    ),
    ...
    specialties = jsonb_build_array('資金調達支援（デット、エクイティー）', 'ピッチブック作成・添削', '海外進出支援', 'VC業務', 'スタートアップ投資')
...
UPDATE rextrix_cfos SET 
    work_experiences = jsonb_build_array(
        ...
        jsonb_build_object(
            ...
            'industry', 'コンサルティング',
            'experience_years', 4,
            'achievements', jsonb_build_array('決算早期化', '管理会計導入支援', 'グループ連結対応'),
            'is_current', false,
            'display_order', 2
        )
    ),
    ...
    availability_conditions = jsonb_build_object(
        'hours_per_week', jsonb_build_object('min', 5, 'max', 10),
    ...
    compensation_details = jsonb_build_object(
        'primary_type', 'hourly',
        'hourly_rate', jsonb_build_object('min', 4000, 'max', 10000),
    ...
    specialties = jsonb_build_array('経営陣向け報告資料の作成', 'ストーリーテリング', '資金繰り管理', '資金調達（金融機関対応含む）', '予実管理の仕組み化', 'KPI設計', '決算早期化', '経理業務の可視化・効率化', '管理会計導入支援', 'M&A時の財務DD', 'PMI初期フェーズ支援')
